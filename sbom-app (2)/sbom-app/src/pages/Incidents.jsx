    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-72 shrink-0 border-r border-border flex flex-col bg-surface">
        <div className="px-4 py-4 border-b border-border">
          <h2 style={{fontFamily:'Syne,sans-serif'}} className="font-bold text-text-primary text-sm">Incident Investigation</h2>
          <p className="text-[10px] text-muted mt-0.5">Quickly identify root cause and affected assets</p>
          ...
          {INCIDENTS.filter(i => !searching || i.title.toLowerCase().includes(searching.toLowerCase())).map(inc => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="font-mono text-[10px] text-muted">{inc.id}</span>
                <span className={`tag-${inc.severity}`}>{inc.severity}</span>
              </div>
              <div className="text-xs text-text-primary font-medium leading-tight">{inc.title}</div>
              <div className="text-[10px] text-muted mt-1">{inc.reported}</div>
            </button>
          ))}
          {/* placeholder incidents */}
          {[1,2,3].map(i => (
            <div key={i} className="px-4 py-3 border-b border-border/50 opacity-40">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 fade-up">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h1 style={{fontFamily:'Syne,sans-serif'}} className="text-xl font-bold text-text-primary">{selected.title}</h1>
              <div className="flex items-center gap-4 mt-1 text-[11px] font-mono text-muted">
                <span>ID: <span className="text-text-secondary">{selected.id}</span></span>
                <span>Reported: {selected.reported}</span>
                <span>Aircraft: <span className="text-accent">{selected.aircraft}</span></span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {/* Timeline */}
            <div className="card">
              <h2 style={{fontFamily:'Syne,sans-serif'}} className="font-semibold text-sm text-text-primary mb-4 flex items-center gap-2">
                <Clock size={13} className="text-accent" /> Investigation Timeline
              </h2>
                ...
                {selected.timeline.map((t, i) => (
                  <div key={i} className="flex gap-3 pb-4 last:pb-0">
                    ...
                    <div className="pb-1">
                      ...
                      <div className="text-xs text-text-secondary mt-0.5">{t.event}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Related systems */}
            <div className="card">
              <h2 style={{fontFamily:'Syne,sans-serif'}} className="font-semibold text-sm text-text-primary mb-4">Related Systems</h2>
              <div className="space-y-2 mb-4">
                {selected.relatedSystems.map((s, i) => (
                  <div key={s} className={`px-3 py-2.5 rounded-lg border text-xs font-medium ${
                    i === 0 ? 'bg-accent/10 border-accent/30 text-accent' : 'bg-bg border-border text-text-secondary'
                  }`}>{s}</div>
                ))}
              </div>
              <div className="pt-3 border-t border-border">
                <div className="text-[10px] text-muted font-mono mb-2 flex items-center gap-1.5">
                  <GitBranch size={10} /> Suggested Investigation Path
                </div>
                <div className="flex items-center gap-1.5 flex-wrap">
                  {selected.path.map((p, i) => (
                    <div key={i} className="flex items-center gap-1.5">
                      <span className="text-[10px] font-mono bg-bg border border-border rounded px-2 py-1 text-text-secondary">{p}</span>
                      {i < selected.path.length - 1 && <ArrowRight size={10} className="text-muted" />}
                    </div>
                  ))}
                </div>
              </div>
