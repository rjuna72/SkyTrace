...
export default function RepositoryPage() {
  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-5">
      <div>
        <h1 style={{fontFamily:'Syne,sans-serif'}} className="text-2xl font-bold text-text-primary">SBOM Repository</h1>
        <p className="text-text-secondary text-xs mt-0.5">View and manage supplier SBOMs (verified externally)</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 card">
          <h2 style={{fontFamily:'Syne,sans-serif'}} className="font-semibold text-sm text-text-primary mb-4 flex items-center gap-2">
            <Upload size={14} className="text-accent" /> Upload / Import SBOM
          </h2>
          <div
            className="border-2 border-dashed border-border hover:border-accent/40 rounded-xl p-8 text-center cursor-pointer transition-all hover:bg-white/[0.02]"
          ...
          >
            ...
            <p className="text-text-secondary text-sm font-medium">Drop files here or browse</p>
            <div className="flex justify-center gap-2 mt-2">
              <span className="tag-info">SPDX</span>
              <span className="tag-info">CycloneDX</span>
            </div>
          </div>
          <p className="text-[10px] text-muted font-mono mt-3 flex items-center gap-1.5">
        ...
        {/* Summary */}
        <div className="card">
          <h2 style={{fontFamily:'Syne,sans-serif'}} className="font-semibold text-sm text-text-primary mb-4">SBOM Summary</h2>
          <div className="text-center mb-4">
            <div style={{fontFamily:'Syne,sans-serif'}} className="text-4xl font-bold text-text-primary">{SBOM_SUMMARY.total.toLocaleString()}</div>
            <div className="text-[10px] text-muted font-mono mt-1">Total SBOMs</div>
          </div>
      ...
      <div className="card">
        <div className="flex items-center gap-3 mb-4">
          <h2 style={{fontFamily:'Syne,sans-serif'}} className="font-semibold text-sm text-text-primary flex-1">SBOM Registry</h2>
            ...
            <tbody className="divide-y divide-border/40">
              {filtered.map(s => (
                <tr key={s.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="py-2.5 pr-4 font-mono text-accent">{s.name}</td>
                  <td className="py-2.5 pr-4 text-text-secondary">{s.supplier}</td>
                  <td className="py-2.5 pr-4 text-text-dim">{s.system}</td>
                  <td className="py-2.5 pr-4 font-mono text-text-secondary">{s.version}</td>
                  ...
                  <td className="py-2.5 pr-4">
                    <div className="flex items-center gap-2">
                      <div className="w-16 bg-bg border border-border rounded-full h-1.5 overflow-hidden">
                        <div className="h-full rounded-full" style={{
                          width:`${s.completeness}%`,
                          background: s.completeness>=85?'#00e5a0':s.completeness>=65?'#ff6b35':'#ff2d55'
                        }} />
                      </div>
                      <span className="font-mono text-text-secondary">{s.completeness}%</span>
                    </div>
                  </td>
                </tr>
              ))}
